...
**最新进展（2026-02-27 Round 1 实装）**：
- 错题系统后端基座已落地：`wrong_questions` 主表 + CRUD/统计命令与 HTTP 路由。
- 周复盘改造为“真实自然周清单”机制：`weekly_review_items` 已落库，支持周内任意勾选与手动延续到下周。
- AI 图片链路后端已打通：`/api/images/upload` 与 `/api/images/{*path}` 已可用于移动端浏览器拍照后上传。
- Settings 已新增视觉模型与识题模式配置（`aiVisionModel` / `aiVisionMode`），AI 客户端支持 `single/pipeline` 两种模式。
- Round 2 将完成 Notes 前端图片发送、一键收录错题、ErrorBook 页面与 Dashboard/WeeklyReview 联动展示。

**最新进展（2026-02-27 Round 2 实装）**：
- Notes 已支持图片题发送（选择图片/粘贴截图）并接入视觉识题流程。
- Notes 助手消息已接入“收录为错题”弹窗，支持同步写入日留痕。
- `ErrorBook` 页面与路由已落地（`/error-book`），支持筛选、详情查看、归档、周清单勾选与下周延续。
- Dashboard 已新增错题速览卡片，支持从首页直达复习。
- WeeklyReview 已接入本周错题标题清单，按真实自然周执行勾选与延续逻辑。
...
| AI 图片识题 | 支持图片题发送（选择/粘贴）并走视觉识题链路 | 🟢 |
| 一键收录错题 | 助手消息可直接收录为错题并可同步到留痕 | 🟢 |
...
| AI (ai) | DeepSeek / Kimi / MiniMax 三家 API Key + 视觉模型与识题模式配置 | 🟢 |
