...
### 2.7 一键导入计划 MD（新增）
- **目标**: 用户上传一份周计划或月计划 Markdown，即自动生成任务。
- **入口**: `Tasks` 页新增「导入计划」按钮。
- **能力**:
  - 解析 `- [ ]` 任务项。
  - 识别 `@YYYY-MM-DD` 日期、`#标签`、优先级标签（`#high/#medium/#low`）。
  - 导入前预览 + 冲突处理（跳过 / 覆盖 / 合并）。
- **与现有组件联动**:
  - 导入后立即出现在 To-Do / 日历 / 甘特图。
  - 首页 Dashboard 的“当日任务勾选”和“签到关联任务”直接消费导入结果。

### 2.8 Tailscale 内网穿透（新增）
- **目标**: 让手机/平板在外网环境也可访问桌面端本地数据。
- **方案**:
  - 桌面端提供 Tailnet 连接状态展示与一键诊断。
  - 通过 MagicDNS/100.x 地址访问本地服务。
  - 权限分级：只读（查看）/读写（编辑），默认只读。
- **安全前提**:
  - 仅授权设备可访问。
  - PRD 约束 ACL、设备过期策略、最小暴露端口。
...
- **任务**:
  ...
  - 计划导入器 MVP：基于 `remark` 解析结构化计划 Markdown 并生成任务草案。
...
- **任务**:
  ...
  - Tailscale 穿透试点：
    - 集成连接检测（`tailscale status`）与失败提示。
    - 配置 Tailnet 访问入口与安全策略预设。
    - 输出多端访问说明（桌面/手机/平板）。
