...
- 归档语义：`wrong_questions.is_archived = 1`，并在归档时同步清理该题对应的 `weekly_review_items`，确保周复盘不再提示。
- 删除语义：新增“硬删除”能力（用于已归档视图），直接删除 `wrong_questions` 行；`weekly_review_items` 通过外键 `ON DELETE CASCADE` 自动级联删除。
- 统计口径修正：周清单与错题统计仅统计 `is_archived = 0` 的错题，避免归档历史继续进入待复习计数。
- 缓存边界：错题正文、归档状态、周清单状态均以 SQLite 为唯一事实源，不写入 localStorage 缓存。
